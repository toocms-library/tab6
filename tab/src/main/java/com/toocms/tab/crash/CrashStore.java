...
import com.toocms.tab.BuildConfig;
...
public final class CrashStore {
    ...
    private static final String CRASH_HANDLER_PACKAGE_NAME = "com.toocms.tab.crash";
                        ...
                        if (config.isEnabled()) {
                            ...
                            if (hasCrashedInTheLastSeconds(application)) {
                                ...
                                if (isStackTraceLikelyConflictive(throwable, errorActivityClass)) {
                                    ...
                                    intent.putExtra(EXTRA_STACK_TRACE, getStackTrace(throwable));
                                    ...
                                    // upload
                                    uploadCrashLog(throwable, null);
                                    ActivityUtils.startActivity(intent, R.anim.scale_enter, R.anim.scale_exit);
    ...
    public static void uploadCrashLog(Throwable throwable, String url) {
        ...
        params.put("banbenmingcheng", getVersionName(application));  // 版本名称
        params.put("create_time", TimeUtils.getNowString());  // 创建时间
        String message = getMessage(throwable);
        params.put("bengkuiyuanyin", message.split(":")[0]); // 崩溃原因
        params.put("flag_md5", EncryptUtils.encryptMD5ToString(message));
        params.put("xiangxixinxi", getAllErrorDetailsFromIntent(TooCMSApplication.getInstance(), throwable, url));  // 堆栈跟踪
        ApiTool.post("http://api.toocms.com/AppLog/addNew")
                .params(params)
    ...
    public static String getStackTraceFromIntent(@NonNull Intent intent) {
        return intent.getStringExtra(CrashStore.EXTRA_STACK_TRACE);
    }
    ...
    public static CrashConfig getConfigFromIntent(@NonNull Intent intent) {
        return (CrashConfig) intent.getSerializableExtra(CrashStore.EXTRA_CONFIG);
    }
    ...
    public static String getActivityLogFromIntent(@NonNull Intent intent) {
        return intent.getStringExtra(CrashStore.EXTRA_ACTIVITY_LOG);
    }
     ...
     * @param object  The Intent/Throwable. Must not be null.
     * @param url
     * @return The full error details.
     */
    @NonNull
    public static String getAllErrorDetailsFromIntent(@NonNull Context context, @NonNull Object object, String url) {
        ...
        errorDetails.append("Build Version: ");
        errorDetails.append(versionName);
        errorDetails.append("<br/>");

        if (buildDateAsString != null) {
            errorDetails.append("Build Date: ");
            errorDetails.append(buildDateAsString);
            errorDetails.append("<br/>");
        }

        errorDetails.append("Date: ");
        ...
        errorDetails.append("Device: ");
        errorDetails.append(getDeviceModelName());
        errorDetails.append("<br/><br/>");

        if (!StringUtils.isEmpty(url)) {
            errorDetails.append("Request Url: ");
            errorDetails.append(url);
            errorDetails.append("<br/><br/>");
        }

        errorDetails.append("Stack Trace:");
        errorDetails.append("<br/>");
        errorDetails.append((object instanceof Throwable) ? getStackTrace((Throwable) object) : getStackTraceFromIntent((Intent) object));
        errorDetails.append("<br/>");
