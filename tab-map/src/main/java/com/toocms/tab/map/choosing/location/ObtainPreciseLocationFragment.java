...
public class ObtainPreciseLocationFragment extends BaseFragment<FgtObtainPreciseLocationBinding, ObtainPreciseLocationViewModel> {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        binding.obtainPreciseLocationMap.onCreate(savedInstanceState);
    }

    @Override
    protected void onFragmentCreated() {
        topBar.setTitle("选择位置");
        // 获取地图缩放等级
        mapZoomLevel = QMUIResHelper.getAttrFloatValue(getBaseFragmentActivity(), R.attr.app_map_zoom_level);
        // 初始化地图控制器
        aMap = binding.obtainPreciseLocationMap.getMap();
        ...
        aMap.setOnCameraChangeListener(new AMap.OnCameraChangeListener() {
            ...
            public void onCameraChangeFinish(CameraPosition cameraPosition) {
                viewModel.result.setLatitude(viewModel.decimalFormat.format(cameraPosition.target.latitude));
                viewModel.result.setLongitude(viewModel.decimalFormat.format(cameraPosition.target.longitude));
    ...
    public void onDestroy() {
        super.onDestroy();
        binding.obtainPreciseLocationMap.onDestroy();
    }

    @Override
    public void onResume() {
        super.onResume();
        binding.obtainPreciseLocationMap.onResume();
    }

    @Override
    public void onPause() {
        super.onPause();
        binding.obtainPreciseLocationMap.onPause();
    }

    @Override
    public void onSaveInstanceState(@NotNull Bundle outState) {
        super.onSaveInstanceState(outState);
        binding.obtainPreciseLocationMap.onSaveInstanceState(outState);
    }
